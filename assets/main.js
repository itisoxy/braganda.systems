...
(function () {
  ...
  const PLATFORM_ICONS = {
    ...
    'Claude (Anthropic)': 'M17.3041 3.541h-3.6718l6.696 16.918H24Zm-10.6082 0L0 20.459h3.7442l1.3693-3.5527h7.0052l1.3693 3.5528h3.7442L10.5363 3.5409Zm-.3712 10.2232 2.2914-5.9456 2.2914 5.9456Z',
    ...
    'GA4': 'M22.84 2.9982v17.9987c.0086 1.6473-1.3197 2.9897-2.967 2.9984a2.9808 2.9808 0 01-.3677-.0208c-1.528-.226-2.6477-1.5558-2.6105-3.1V3.1204c-.0369-1.5458 1.0856-2.8762 2.6157-3.1 1.6361-.1915 3.1178.9796 3.3093 2.6158.014.1201.0208.241.0202.3619zM4.1326 18.0548c-1.6417 0-2.9726 1.331-2.9726 2.9726C1.16 22.6691 2.4909 24 4.1326 24s2.9726-1.3309 2.9726-2.9726-1.331-2.9726-2.9726-2.9726zm7.8728-9.0098c-.0171 0-.0342 0-.0513.0003-1.6495.0904-2.9293 1.474-2.891 3.1256v7.9846c0 2.167.9535 3.4825 2.3505 3.763 1.6118.3266 3.1832-.7152 3.5098-2.327.04-.1974.06-.3983.0593-.5998v-8.9585c.003-1.6474-1.33-2.9852-2.9773-2.9882z',
  ...
  const TECH_PLATFORMS = {
    'ai-gtm': ['HubSpot', 'Salesforce', 'Clay', 'Apollo', 'n8n', 'OpenAI', 'Claude (Anthropic)'],
    'ai-outbound': ['Clay', 'Apollo', 'Apify', 'n8n', 'OpenAI', 'Claude (Anthropic)'],
    'gtm-intelligence': ['Clay', 'Apollo', 'Apify', 'OpenAI', 'Claude (Anthropic)', 'Slack'],
    'crm-revops': ['HubSpot', 'Salesforce', 'Pardot', 'Zoho CRM', 'n8n', 'Zapier'],
    'ai-automation': ['n8n', 'Zapier', 'Make', 'OpenAI', 'Claude (Anthropic)', 'Slack'],
    'custom-ai': ['Supabase', 'PostgreSQL', 'AWS', 'OpenAI', 'Claude (Anthropic)', 'Figma'],
  };

  function renderPlatforms(container) {
    const slug = container.dataset.platforms;
    const names = TECH_PLATFORMS[slug];
    if (!names) return;
    container.innerHTML = `<div class="tech-row">${names.map(name => {
      const tech = TECH_DATA.find(t => t.name === name);
      ...
      return `
        <div class="tech-badge platform-badge" tabindex="0">
          ${mark}${name}
          <span class="tech-tip" role="tooltip">${desc}</span>
        </div>
      `;
    }).join('')}</div><p class="tech-disclaimer">All product names, logos and brands are property of their respective owners. Their use does not imply endorsement or partnership. Displaying a technology means we work with, understand or can integrate it.</p>`;
  }

  document.querySelectorAll('[data-platforms]').forEach(renderPlatforms);
})();
